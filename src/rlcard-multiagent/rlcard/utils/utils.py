    ...
    for player in range(num_players):
        for i in range(0, len(trajectories[player])-2, 2):
            if i ==len(trajectories[player])-3:
                ...
                #False is for fully competitive case
                # if False:
                    player_rewards = [payoffs[p] for p in range(num_players)]
                    winners = np.flatnonzero(player_rewards == np.max(player_rewards)).tolist()
                    
                    assert len(winners) > 0

                    if len(winners) == 1 and (winners[0] == 0 or winners[0] == 1):
                        #One Collab agent wins
                        reward = max(player_rewards[0], player_rewards[1])
                    elif len(winners) == 1:
                        #Collab agents lose
                        reward = (player_rewards[0] + player_rewards[1]) / 2
                    elif len(winners) == 2 and (max(winners) != 2):
                        #Collab agents tie win
                        reward = (player_rewards[0] + player_rewards[1]) / 2 
                    elif len(winners) == 2: 
                        #Collab agent + Single win
                        reward = max(player_rewards[0], player_rewards[1]) #ambiguous case, need more clarification
                    else:
                        #All win
                        reward = (player_rewards[0] + player_rewards[1]) / 2                      
